...
## @params: [JOB_NAME]
args = getResolvedOptions(sys.argv, ['ETL_redes_sociais'])
...
job = Job(glueContext)
job.init(args['ETL_redes_sociais'], args)

# Gerando um DynamicFrame a partir de uma tabela no Data Catalog
redes_sociais_zoop_dyf = glueContext.create_dynamic_frame.from_catalog(database='db-glue-dados', table_name='dados-glue-redes_sociais_zoop_bronze_parquet')
...
s3output = glueContext.getSink(
  path="s3://dados-zoop-cie/silver/redes-sociais/",   # adicione o nome do seu bucket no path
  connection_type="s3",
...
s3output.setCatalogInfo(
  catalogDatabase="db-glue-dados", catalogTableName="dados-glue-redes_sociais_zoop_silver"
)
